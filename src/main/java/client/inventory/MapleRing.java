...
public class MapleRing implements Serializable {
      ...
      try {
         ...
         try (PreparedStatement ps = con.prepareStatement("SELECT * FROM rings WHERE ringId = ?")) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
               ret = null;
               if (rs.next()) {
                  ret = new MapleRing(ringId, rs.getInt("partnerRingId"), rs.getInt("partnerChrId"), rs.getInt("itemid"),
   ...
   public static int[] makeRing(int itemid, MapleCharacter partner1, MapleCharacter partner2) throws
         Exception { //return partner1 the id
   ...
   public static int makeRing(int itemid, MapleCharacter partner1, String partner2, int id2, String msg, int sn) throws
         Exception { //return partner1 the id
      int[] ringID = {MapleInventoryIdentifier.getInstance(), MapleInventoryIdentifier.getInstance()};
      //[1] = partner1, [0] = partner2
      try {
